...
2. Require the epic status to be exactly `active`. If it is not active, stop
   and explain that a human must update the manifest; never change the
   manifest automatically.
3. Verify the epic is listed by the milestone and that the milestone points
   back to the epic.
4. Check every `depends_on` epic. Each dependency must exist and have status
   `completed`; otherwise stop before any Git or runtime-state write.
5. Read the current branch with:

   ```powershell
   git branch --show-current
   ```

6. Verify that `base_branch` exists locally:
...
7. If the epic branch does not exist locally, create and check it out from the
   local base branch:
   ...
   operations.
8. Create the ignored runtime directory if needed and write only the epic ID
    with UTF-8 text to:
...
`SAFE_TO_RUN_LOOP: yes` is allowed only when the manifest checks passed, the
epic is active, all dependencies are completed, the branch matches the
manifest, and the runtime file was written successfully. A successful start
does not authorize the single-task loop to skip its own preflight, finalize,
