...
class LocalArtifactStore(StreamingArtifactStore):
    ...
    def __init__(self, root: Path | str, *, index: ProjectArtifactIndex | None = None) -> None:
        ...
        if index is None and (self._manifest_root / "index.sqlite").exists():
            raise ValueError("Project artifacts require LocalArtifactStore.for_project(repository).")
        self._staging = self._manifest_root / "staging"
    ...
    def _import_stream(self, name, source, metadata):
        ...
        stage = Path(tempfile.mkdtemp(prefix="publication-", dir=self._staging))
        _write_json(stage / "intent.json", manifest.to_payload())
        digest, size = hashlib.sha256(), 0
        with (stage / "payload").open("xb") as output:
            while True:
            ...
            output.flush()
            os.fsync(output.fileno())
        if (stage / "payload").stat().st_size != size:
        ...
        _write_json(stage / "complete.json", manifest.to_payload())
        os.replace(stage / "complete.json", stage / "intent.json")  # Private journal only.
        artifact_path.parent.mkdir(parents=True, exist_ok=True)
        _publish_file(stage / "payload", artifact_path)
    ...
    def _artifact_path(self, key: str) -> Path:
        ...
        if any(":" in part for part in PurePosixPath(normalized).parts):
            raise ValueError("Artifact key must not contain drive or stream syntax.")
    ...
    def _registered(self) -> tuple[ArtifactManifest, ...]:
        ...
        return tuple(ArtifactManifest.from_payload(json.loads(path.read_text(encoding="utf-8")))
    ...
    def recover(self) -> tuple[PublicationRecovery, ...]:
        ...
        report = []
        for stage in sorted(self._staging.glob("publication-*")):
            if not stage.is_dir() or stage.is_symlink():
                continue
            stage.resolve().relative_to(self._staging.resolve())
            key = None
            try:
                manifest = ArtifactManifest.from_payload(json.loads((stage / "intent.json").read_text(encoding="utf-8")))
