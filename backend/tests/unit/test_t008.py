...
def test_core_workflow_engine_executes_in_order_and_skips_disabled_modules() -> None:
    order: list[str] = []
...
def test_core_workflow_engine_stops_when_a_required_dependency_is_missing() -> None:
    order: list[str] = []
    registry = ModuleRegistry(
        [
            _definition("outline"),
...
def test_core_workflow_engine_records_failed_module_execution() -> None:
    order: list[str] = []
    registry = ModuleRegistry([_definition("brief")])
    modules = {
        "brief": RecordingModule(
            definition=_definition("brief"),
            order=order,
