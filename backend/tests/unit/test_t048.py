...
def test_narration_fixture_discovery_matches_metadata() -> None:
    ...
    expected = sorted(FIXTURE_DIR / entry["file"] for entry in FIXTURE_ENTRIES)

    assert len(discovered) == 5, (
        f"narrations: expected 5 UTF-8 story fixtures, found {len(discovered)} "
...
def test_narration_metadata_has_expected_shape() -> None:
    ...
    assert FIXTURE_METADATA["language"] == "multi", (
        f"metadata.json: language expected 'multi', got {FIXTURE_METADATA['language']!r}"
    )
    ...
    assert len(FIXTURE_ENTRIES) == 5, (
        f"metadata.json: expected 5 fixture entries, found {len(FIXTURE_ENTRIES)}"
    )
...
def test_narration_metadata_entries_point_to_valid_utf8_fixtures(entry: dict[str, object]) -> None:
    ...
    assert entry["language"] in {"en", "pl"}, (
        f"{file_name}: language expected 'en' or 'pl', got {entry['language']!r}"
    )
...
def test_narration_fixture_contents_include_required_linguistic_signals(entry: dict[str, object]) -> None:
    ...
    assert any(mark in text for mark in ('„', '“', '"')), (
        f"{file_name}: content - missing quoted dialogue"
    )
