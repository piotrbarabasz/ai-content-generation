...
- [X] CHK002 Does the workflow engine depend on provider abstractions rather than concrete vendors or implementations? [Consistency, Spec §FR-020, FR-021]
...
- [X] CHK005 Are providerConfig requirements defined without embedding provider-specific logic into the workflow engine? [Clarity, Spec §FR-020]
- [X] CHK006 Are provider settings validated before workflow execution without coupling to a concrete backend? [Coverage, Spec §FR-022]
- [X] CHK007 Are the requirements clear that external providers may be replaced behind the same contract? [Clarity, Spec §FR-020, FR-021]

## Scope and Isolation

- [X] CHK008 Does the MVP avoid requiring real provider credentials for core workflows? [Scope, Spec §SC-006]
