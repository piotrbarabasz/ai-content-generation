...
- **Status:** Completed — approved private Windows/CUDA distribution, installed selection and measured EN/PL GPU acceptance PASS (2026-09-22)
- **Milestone:** M10
- **Priority:** P1
...
- **Implementation boundary:** The Piper-only D045 v1 distribution contract remains unchanged. Chatterbox V3 uses its own exact Windows x64/CUDA 12.4 closure and allowlist, preserving source commit `5de7a54aa4e5e2baadb0182dde554908b48b85c2` despite the shared PyPI version `0.1.7`. Provisioning consumes reviewed artifacts into a private embedded Python 3.11.9 runtime, verifies the complete installed inventory and activates only after fixed health passes. Installed-product selection is explicit and does not replace Piper. Reference-audio intake stays in D030; no automatic CPU fallback or network access during synthesis.
- **Evidence:** [D029 managed Chatterbox runtime](D029_MANAGED_CHATTERBOX.md) records the reviewed dependency/native/license closure, tokenizer provisioning, activation/restart result and retained real-hardware evidence. Large runtime/model/project/audio evidence remains outside Git.
- **Validation:** Focused D029 distribution/profile/health, provisioning, section/preview and installed-selection tests: 55 passed. Real Windows/GPU acceptance on NVIDIA GeForce GTX 1660 SUPER: EN 2.80 s audio, PL 7.92 s audio, interrupted PL resume reused 1 of 3 chunks, preview/production contention blocked, unload/restart and post-exit ownership PASS; peak reserved VRAM 3,458,203,648 B (EN) and 3,477,078,016 B (PL). Full `python -m pytest backend/tests`: 1640 passed, 11 skipped in 367.65 s (isolated Python 3.11.9). `python -m compileall -q backend/app backend/tests packaging/d029`, `python -m pip check` and `git diff --check` PASS.
...
- **Validation:** Focused implementation checks: 98 passed. Final full `python -m pytest backend/tests -o addopts='' -q --tb=short`: 1650 passed, 11 existing optional tests skipped in 768.09 s (2026-09-23, Windows, isolated Python 3.11); exit code 0. `compileall`, `pip check` and `git diff --check` PASS. D021's existing manual desktop smoke remains a separate dependency gate.
