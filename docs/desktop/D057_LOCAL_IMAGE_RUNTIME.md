...
The GTX 1660 SUPER reports 6143.6875 MiB to PyTorch. The managed runtime installed and verified, but the previous exact-byte 6 GiB preflight rejected this device before inference. The bounded-tolerance fix requires a newly verified managed installation and another real smoke. No real image, generation time, peak VRAM or post-real-inference restart measurement is claimed. First visual prompt creation still needs explicit brief/style context; Film Brief and Visual Style UI are outside D057.
